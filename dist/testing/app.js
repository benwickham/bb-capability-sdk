...
function normalizeCapabilityThreadRowStatus(value, onRejected) {
...
function useSlotEnv(hook) {
  ...
  if (!env) {
    throw new Error(
      `${hook}() needs the test slot environment \u2014 mount the component via renderSlot(...) from @get-bb/capability-sdk/testing/app`
    );
  }
  return env;
}
function defineCapabilityApp(setup) {
  if (typeof setup !== "function") {
    throw new Error("defineCapabilityApp expects a setup function");
  }
  return Object.freeze({ __bbCapabilityApp: true, setup });
}
function isCapabilityAppDefinition(value) {
  return typeof value === "object" && value !== null && value.__bbCapabilityApp === true && typeof value.setup === "function";
}
          ...
          "button",
          {
            ...
            onClick: () => {
              void onSubmit({
                ...
                model: defaultModel ?? "openai-codex/gpt-5.6-sol",
...
function installTestCapabilityRuntime() {
  const host = globalThis;
  host.__bbCapabilityRuntime = {
    ...host.__bbCapabilityRuntime,
    capabilitySdkApp: testCapabilitySdkApp
  };
}
async function loadCapabilityApp(source) {
  installTestCapabilityRuntime();
  const resolved = typeof source === "function" ? await source() : source;
  const definition = isCapabilityAppDefinition(resolved) ? resolved : resolved.default;
  if (!isCapabilityAppDefinition(definition)) {
    throw new Error(
      "the bundle's default export is not defineCapabilityApp(...) from @get-bb/capability-sdk/app"
    );
  }
  return collectCapabilityAppRegistrations(definition);
}
async function mountCapabilityContentScripts(app, options) {
  ...
  const setThreadRowStatus = (threadId, status) => {
    ...
    if (typeof threadId !== "string" || threadId.trim().length === 0) {
      console.warn(
        `bb capability "${options.capabilityId}": contentScript.experimental_setThreadRowStatus: "threadId" must be a non-empty string`
      );
      return;
    }
    const normalizedThreadId = threadId.trim();
    const normalizedStatus = normalizeCapabilityThreadRowStatus(
      status,
      (reason) => console.warn(`bb capability "${options.capabilityId}": ${reason}`)
    );
  ...
  const dispose = async () => {
    ...
    for (const script of [...mounted].reverse()) {
      ...
      try {
        ...
        console.warn(
          `[capability:${options.capabilityId}] content script "${script.id}" cleanup failed: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
...
function renderSlot(registration, props, options = {}) {
  ...
  const navigate = {
    ...
    toCapabilityPanel(path, panelOptions) {
      navigateCalls.push({
